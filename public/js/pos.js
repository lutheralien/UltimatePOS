...
$(document).ready(function() {
    ...
    $('button.pos-express-finalize').click(function() {
        ...
        if (pay_method == 'card') {
            ...
            var formData = pos_form_obj.serializeArray();
            ...
            function storeTransaction(data) {
                ...
                toastr.warning('Transaction stored offline. Will sync when online.');
                reset_pos_form();
            }
            ...
            if (navigator.onLine) {
                ...
                storeTransaction(formData);
            }
        }
    });
    
    function processOfflineTransactions() {
        let offlineTransactions = JSON.parse(localStorage.getItem('offlineTransactions') || '[]');
        if (offlineTransactions.length === 0) return;
    
        offlineTransactions.forEach((transaction, index) => {
            if (transaction.status === 'pending') {
                // Convert transaction data to match Laravel expectations
                let formData = new FormData();
                
                // Add required fields from Laravel store method
                formData.append('location_id', pos_form_obj.find('[name="location_id"]').val());
                formData.append('status', 'final');
                formData.append('is_quotation', 0);
                
                // Add products data
                transaction.data.forEach(item => {
                    formData.append(item.name, item.value);
                });
    
                // Add payment details
                if (transaction.data.find(item => item.name === 'payment')) {
                    let payment = transaction.data.find(item => item.name === 'payment');
                    formData.append('payment', JSON.stringify(payment.value));
                }
    
                $.ajax({
                    method: 'POST',
                    url: pos_form_obj.attr('action'),
                    data: formData,
                    processData: false,
                    contentType: false,
                    async: false,
                    success: function(result) {
                        if (result.success) {
                            offlineTransactions = offlineTransactions.filter(t => t !== transaction);
                            localStorage.setItem('offlineTransactions', JSON.stringify(offlineTransactions));
                    ...
                    error: function(xhr) {
                        toastr.error(xhr.responseJSON?.msg || 'Transaction failed');
                    }
                });
            }
        });
    ...
    window.addEventListener('online', processOfflineTransactions);
